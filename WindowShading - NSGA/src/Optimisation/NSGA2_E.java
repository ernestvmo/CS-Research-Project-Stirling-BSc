...
//		System.out.println("TEST OFFSPRING " + offspring[0].getFitness1() + " " + offspring[0].getFitness2());
		
		for (Individual i : initial) {
			System.out.println(i.getFitness1() + " " + i.getFitness2());
		}
		
		double f = regression.Plotting.hypervolume(initial);
...
//			for (Individual t : R) {
//				System.out.println("INDIVS IN R : " + t.getFitness1() + " " + t.getFitness2() + " " + t.toString());
//			}
...
//			System.out.println("TESTTESTTES  R " + R[initial.length].getFitness1() + " " + R[initial.length].getFitness2());
...
//			System.out.println("eval: " + currentEval);
		...
		double l = regression.Plotting.hypervolume(initial);
...
//		int i = 0;
//		for (List<Individual> fi = fronts.get(i); fi.size() > 0; fi = fronts.get(i))
//		{
...
//		return fronts;
	...
	{
		Individual[] I = individuals.toArray(new Individual[individuals.size()]);
		...
		for (int j = 1; j < l - 1; j++)
		{
			I[j].distance += (I[j + 1].getFitness1() - I[j - 1].getFitness1()) / fitness1Range;
		}
		...
		for (int j = 1; j < l - 1; j++)
		{
			I[j].distance += (I[j + 1].getFitness2() - I[j - 1].getFitness2()) / fitness2Range;
		}
