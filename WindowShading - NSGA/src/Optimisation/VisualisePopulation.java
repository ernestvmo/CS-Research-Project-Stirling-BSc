...
public class VisualisePopulation extends JFrame {
    ...
    public VisualisePopulation(double minFitness1, double maxFitness1, double minFitness2, double maxFitness2) {
    ...
    public void setSize(int width, int height) {
        ...
        this.pointWidth = Math.max(1, (int)(width * POINT_SIZE_AS_FRACTION_OF_WINDOW_SIZE));
        this.pointHeight = Math.max(1, (int)(height * POINT_SIZE_AS_FRACTION_OF_WINDOW_SIZE));
    }
...
//      this.validate();
    ...
    public void paint(Graphics g) {
        ...
        bounds.setLocation(0, 0); // okay, we don't want to include the window's coords for internal drawing!
        ...
        // trim the bounds to omit the axes (y coord is same as that's at top of window)
        Rectangle trimmedBounds = new Rectangle(bounds.x + axisOffset, bounds.y, bounds.width - axisOffset, bounds.height - axisOffset);

        // add tic marks
        int textHeight = 20;
        int textWidth = 40;
        @SuppressWarnings("unused")
		int upperOffset = 30; // height of title bar - isn't "bounds" supposed to account for this?
        //g2.drawString(Double.toString(minFitness1), bounds.x + axisOffset, textHeight + bounds.y + (bounds.height - axisOffset));
        //g2.drawString(Double.toString(maxFitness1), bounds.x + bounds.width - textWidth, textHeight + bounds.y + (bounds.height - axisOffset));
        //g2.drawString(Double.toString(minFitness2), bounds.x + axisOffset - textWidth, bounds.y + (bounds.height - axisOffset));
        //g2.drawString(Double.toString(maxFitness2), bounds.x + axisOffset - textWidth, bounds.y + textHeight + upperOffset);

        // points on axes (tics)
        int numXTics = 5;
        int numYTics = 5;
        int ticSize = 3;
        // NB- after this we pretend there is an extra tic at the extremes of each of the axes, but don't show it as it'd be clipped
        ...
        for (int i = 0; i < numXTics; i++) {
            ...
            // the text
            g2.drawString(Double.toString(roundDouble(curFit, 2)), ticX, xAxisYPos + textHeight);
            curFit += xTicInterval;
        }
        ...
        for (int i = 0; i < numYTics; i++) {
            ...
            // the text
            g2.drawString(Double.toString(roundDouble(curFit, 2)), yAxisXPos - textWidth, ticY);
            curFit += xTicInterval;
        }
        ...
        if (this.population != null) {
            for (Individual i : this.population) {
                ...
                // normalise fitnesses
                double n1 = (i.getFitness1() - minFitness1) / (maxFitness1 - minFitness1);
                double n2 = (i.getFitness2() - minFitness2) / (maxFitness2 - minFitness2);
...
//                System.out.println("B:" + i.getFitness2() + ":" + n2);
                ...
                int y = trimmedBounds.y + trimmedBounds.height - (int)(trimmedBounds.height * n2);
    ...
    private double roundDouble(double d, int decimalPlaces) {
        BigDecimal round = new BigDecimal(d).setScale(decimalPlaces,BigDecimal.ROUND_HALF_UP);
